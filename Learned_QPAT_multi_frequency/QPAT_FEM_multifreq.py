...
def Form_V(geom,file_fff,file_fdd):
    ...
    if os.path.isfile(file_fff):
        ...
        V_fdd_torch = torch.from_numpy(V_fdd[0].todense()).to_sparse()
        V_fff_torch = torch.from_numpy(V_fff[0].todense()).to_sparse()

        
        for mat in range(n-1):
            V_temp = torch.from_numpy(V_fdd[mat+1].todense()).to_sparse()
            V_fdd_torch = torch.cat((V_fdd_torch,V_temp),dim=0)
    
            V_temp = torch.from_numpy(V_fff[mat+1].todense()).to_sparse()    
